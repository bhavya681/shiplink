...
const GetAllShippers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (data.success) {
        setUsers(data.users.filter((user) => user.role === "Shipper"));
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen pt-20 bg-gray-50">
      {/* Mobile header */}
      <div className="md:hidden flex items-center justify-between p-4 bg-white border-b border-gray-200">
        ...
        <>
          ...
          <h1 className="text-xl font-bold text-gray-800">User Chats</h1>
        )}
      ...
      <div className={`${isMobileSidebarOpen ? 'flex' : 'hidden'} md:flex w-full md:w-80 lg:w-96 flex-col border-r border-gray-200 bg-white transition-all duration-300 ease-in-out`}>
        ...
        <div className="p-4 bg-white sticky top-0 z-10 border-b border-gray-200">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800 mb-4 hidden md:block text-center items-center">Users</h1>
      ...
      <div className={`${!isMobileSidebarOpen || !isMobile ? 'flex' : 'hidden'} flex-1 flex-col bg-white`}>
        {selectedUser ? (
          <>
            <div className="border-b border-gray-200 p-4 py-4 flex items-center md:hidden">
              <button 
                onClick={handleBackToUserList}
